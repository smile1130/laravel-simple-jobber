...
// Class definition
var KTUsersAddMarkup = function () {
    // Shared variables
    const element = document.getElementById('kt_modal_add_markup');
    const form = element.querySelector('#kt_modal_add_markup_form');
            ...
            form,
            {
                fields: {
                    'markup_name': {
                        validators: {
                            notEmpty: {
                                message: 'Markup name is required'
                            },
                    ...
                    'markup_rate': {
                        validators: {
                            notEmpty: {
                                message: 'Markup rate is required'
                            },
                            between: {
                                min: 1,
                                max: 100,
            ...
            if (validator) {
                validator.validate().then(function (status) {
                    ...
                    if (status == 'Valid') {
                        ...
                        const markupName = $(element.querySelector('[name="markup_name"]')).val();
                        const markupRate = $(element.querySelector('[name="markup_rate"]')).val();
								...
								if (response.success) {
									Swal.fire({
										...
										customClass: {
                                            ...
                                            let markupInput = document.getElementById("markup_list");
                                            let newHtmlContent = `<div class="d-flex align-items-center position-relative mb-7" id="markup_${response.id}">
                                                    <div class="position-absolute top-0 start-0 rounded h-100 bg-secondary w-4px"></div>
                                                    <div class="fw-bold ms-5">
                                                        <a href="#" class="fs-5 fw-bolder text-dark text-hover-primary">${markupName}</a>
                                                        <div class="fs-7 text-muted">rate: ${markupRate}</div>
                                                    </div>
